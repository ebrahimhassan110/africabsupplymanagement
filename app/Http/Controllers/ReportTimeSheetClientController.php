...
use Carbon\Carbon;
use App\Models\User;
class ReportTimeSheetClientController extends Controller
{
    public function index(){
      ...
      $workplan_report_data = Workplan::with('Employee')->with('customer')->with('worktype')->whereDate('created_at',[$first,$end])->orderBy('created_at', 'desc')->get();
      $employees =  User::all();
       $customers =  Customer::all();
      $statuses = \DB::table('workplan_status')->get();
      $status="ttt";
      return view('report.timesheetclient.index',compact('workplan_report_data','employees','statuses','_start_date','_end_date','customers','first','end','status'));
    }
